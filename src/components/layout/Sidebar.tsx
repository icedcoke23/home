...
export default function Sidebar() {
  ...
  return (
    <>
      <div className="fixed inset-0 bg-black/40 z-40 animate-fade-in" onClick={toggleSidebar} aria-hidden />
      ...
      <aside className="fixed left-0 top-0 bottom-0 w-72 bg-[var(--ds-bg-secondary)] z-50 animate-slide-in shadow-2xl flex flex-col">
        ...
        {/* New chat */}
        <button onClick={() => { createConversation(); setPage('chat'); toggleSidebar(); }}
          className="mx-3 mt-3 py-2.5 rounded-xl border border-dashed border-[var(--ds-border)] text-sm
                     text-[var(--ds-accent)] hover:bg-[var(--ds-bg-card)] transition-all duration-150 active:scale-95">
          + 新对话
        </button>
            ...
            <div className="flex flex-col gap-1">
              {conversations.map((conv) => (
                <div key={conv.id}
                  ...
                  onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); selectConv(conv.id); } }}
                  aria-label={`对话: ${conv.messages[0]?.content.slice(0, 20) || conv.title}`}
                >
                  ...
                  <button onClick={(e) => { e.stopPropagation(); deleteConversation(conv.id); }}
                    className="opacity-0 group-hover:opacity-100 text-[var(--ds-text-muted)] hover:text-[var(--ds-accent-warm)] p-0.5 transition-all duration-150"
                    aria-label="删除对话">
        ...
        <div className="px-4 py-3 border-t border-[var(--ds-border)]">
          <button onClick={() => { setPage('settings'); toggleSidebar(); }}
            className="w-full flex items-center gap-2 px-3 py-2.5 rounded-lg text-xs text-[var(--ds-text-muted)]
                       hover:bg-[var(--ds-bg-input)] hover:text-[var(--ds-text-primary)] transition-colors duration-150">
